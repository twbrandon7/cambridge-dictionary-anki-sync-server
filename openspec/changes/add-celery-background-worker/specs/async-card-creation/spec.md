...
### Requirement: Card Creation API (Modified)
The `POST /api/v1/clozeNotes` endpoint behavior SHALL change to async-by-default with optional backward-compatible sync mode.
...
### Scalability
Celery workers can be scaled horizontally by adding more worker processes or machines, each consuming tasks from the shared Redis broker.

### Latency
API response time for card creation requests is reduced to <100ms (from 30-60+ seconds with sync model).

### Reliability
Failed tasks are retried automatically; client can poll task status to determine eventual success or failure.

### Backward Compatibility
Sync mode via `async=false` query parameter ensures existing clients continue to work without modification.
