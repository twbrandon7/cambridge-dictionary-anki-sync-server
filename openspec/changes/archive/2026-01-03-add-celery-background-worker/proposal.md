...
## Why
Currently, the card creation API endpoint is synchronous and blocks clients for 30+ seconds during Anki operations (card creation, media generation, AnkiWeb sync). This causes API timeout risks, poor user experience, scalability issues with concurrent requests, and difficult error recovery. Implementing async processing via Celery decouples the request from long-running operations, improving responsiveness, reliability, and scalability.

## What Changes
- Add Celery task queue with SQLite file-based broker and result backend
- Implement `add_cloze_note_task` Celery task for async card creation with error handling
- Modify `POST /api/v1/clozeNotes` to return 202 Accepted with task ID (default) or 200 OK (legacy mode via `?async=false`)
- Add `GET /api/v1/tasks/<task_id>` endpoint for task status polling
- Add task status serialization module for Celery result backend integration
- Update Docker Compose with Celery worker service
